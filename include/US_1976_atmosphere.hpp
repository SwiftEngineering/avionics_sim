...
class Geopotential_height {
  ...
  protected:
    constexpr Geopotential_height(const double geomet_height, const double geopot_height) :
        m_geometric_height(geomet_height),
        m_geopotential_height(geopot_height) {
    }
...
/// Performance above MESOSPHERE2 is degraded, about 5 ULP of error
/// (5/10000 Pa for pressure, 5/10000 kg/m^3 for density)
...
class US_1976_atmosphere {
  ...
  protected:
    ...
    constexpr static double base_temp[] = {288.15, 216.65, 216.65, 228.65, 270.65, 270.65, 214.65, 186.946};  // in Kelvin // NOLINT
    constexpr static double base_pressure[] = {101325.0, 22632.06, 5474.889, 868.0187, 110.9063, 66.93887, 3.956720, 0.3733836};  // in Pa //NOLINT

    // m/(kmol K) - NASA TR R-459 pg 4 - nonstandard definition for US atmosphere 1976 used by convention
    constexpr static double R_star = 8.31432e3;
    constexpr static double M_0 = 28.9644;  // kg/kmol - NASA TR R-459 pg 17 -

    // m/s/s - NASA TR R-459 pg 4 - 1901 value for "45 degrees" / but really closer to 45deg 32min 33s
    constexpr static double g_0 = 9.80665;
