...
class AvionicsSimException: public std::exception {
  public:
    ...
    ///             with the caller. The explicit keyword here is used to prevent implicit conversion of single
    ///             argument.
...
// TODO(Ihimu): Had issues having templatized member object, so must always pass in as first parameter to try-catch.
// In the future, consider a workaround.
class AvionicsSimTryCatchBlock {
  public:
    ...
    ///
    /// \details    Should be used in cases when there is nothing specific to do in catch statement other than get
    ///             error message and criticality. Currently only accepts methods with no parameters. Designed to be
    ///             used in a method that calls several functions that require being called in a try-catch.
    ...
    /// \param[in]  exceptionOccurred  true if exception occurred, false otherwise
    ...
    /// \brief      N/A
    ///
    /// \details    Sets initial exception message. Good for preparing initial data regarding try catch block
    ///             (such as name of function calling the blocks)
    /// \param[in]  std::string     string containing initial exception message.
    /// \return     N/A
    void setExceptionMessage(std::string msg) {
        exceptionMsg = msg;
    }
