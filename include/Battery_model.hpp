...
public:
    ...
    {
        double soh;                     ///< for the eventual addition of state of health.  As SOH declines, internal resistance rises.  This allows interpolating between curves once SOH is supported.
    ...
    /// \return             		    Returns voltage per cell corresponding to state of charge on discharge curve at index value.
    ...
    /// \param [in] ir_coeff    	    pointer to a std::vector that contains the coefficients for the SOC vs. internal resistance curve
    /// \return             		    Returns true if successful false otherwise.
...
protected:
    ...
    double m_r {0.040};                             ///< Equivelent internal resistance for transient response in ohms, must not be zero
    double m_c {0};                             ///< Equivelent capacitance for transient response in Farads
    ...
    uint8_t m_num_discharge_curves;                         ///< Number of discharge curves that have been initialized.  TODO: this is not necessary, refactor, std::vector has a size() method.
    std::vector<internal_resistance_curve> m_internal_resistance_curve;     ///<  Vector of internal resistance curves
    size_t m_internal_resistance_num_coef {11};                             ///<  Array of polynomial coefficients the describe a 10th order polynomial of the internal resistance.
