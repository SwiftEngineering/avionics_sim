...
class Motor_current_consumption_model {
  public:
    ...
    // TODO(Mike Lyons) do we want to set a valid range?
    explicit Motor_current_consumption_model(const std::vector<double> &current_consumption_model_coeff);

    /// \brief Constructor for computing current from torque with dynamic response
    /// \param [in] ccm vector of coefficients from c0 -> c??  should have at least two
    // TODO(Mike Lyons) do we want to set a valid range?
    Motor_current_consumption_model(const std::vector<double> &current_consumption_model_coeff, const double C,
                                    const double R);

    /// \brief Returns expected current at a given rpm or torque based on the current consumption model
    /// \details does not handle dynamic response
    ...
    /// \brief Returns expected current at a given rpm or torque based on the current consumption model
    /// \details includes dynamic response
    /// \param [in] value                       Rotational velocity in RPM or torque in in-lbs
    /// \return                                 Returns current in Amps.
  ...
  protected:
    ...
    double m_r;                 ///< Equivelent resistance for RC type transient response (determine from step data)
    double m_c;                 ///< Equivelent capacitance for RC type transient response (determine from step data)
