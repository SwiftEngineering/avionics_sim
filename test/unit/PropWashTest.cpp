...
TEST(Prop_wash_UnitTest,
     Test_hover_exit_velocity_from_motor_thrust_with_poly_input) {
  for (size_t i = 0; i < hover_thrust_newtons.size(); i++) {
    ASSERT_NEAR(pf.get_output_from_poly(kDefaultPropWashHoverPoly,
                                        hover_thrust_newtons[i]), hover_exit_velocity[i], 2E-1);
  }
}

/// \brief Test getting hover exit velocity from motor thurst input and inputting the poly as well
TEST(Prop_wash_UnitTest,
     Test_cruise_exit_velocity_from_motor_thrust_with_poly_input) {
  for (size_t i = 0; i < cruise_thrust_newtons.size(); i++) {
    ASSERT_NEAR(pf.get_output_from_poly(kDefaultPropWashCruisePoly,
                                        cruise_thrust_newtons[i]), cruise_exit_velocity[i], 2E-1);
  }
}
