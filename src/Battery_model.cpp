...
bool Battery_model::set_internal_resistance_coefficients(const internal_resistance_curve &ir_coeff) {
  ...
  m_internal_resistance_curve.push_back(
    ir_coeff);  // using a vector allows support for multiple internal resistance curves, which will change with state of health
  return true;
}
...
double Battery_model::compute_internal_resistance(double soc, double soh) {
  const uint8_t curve =
    0;  // allows adding additional internal resistance curves for different states of health, currently only one curve is supported
