...
double Differential_pressure_sensor_model::diff_press_from_indicated_airspeed_compressable_flow(
    const double indicated_airspeed) {
    return (pow((indicated_airspeed / CONSTANTS_SPEED_OF_SOUND_AT_SEA_LEVEL) * (indicated_airspeed /
                CONSTANTS_SPEED_OF_SOUND_AT_SEA_LEVEL) / 5.0 + 1.0,
                7.0 / 2.0) - 1.0) * US_1976_atmosphere::CONSTANTS_STATIC_AIR_PRESSURE_AT_SEA_LEVEL;
}
...
double Differential_pressure_sensor_model::corrected_airspeed_from_true_airspeed(const double true_airspeed,
        const double static_pressure, const double temperature_celsius) {
    return true_airspeed / sqrt(
               (US_1976_atmosphere::CONSTANTS_AIR_DENSITY_SEA_LEVEL_15C / get_air_density(static_pressure,
                       temperature_celsius)));
}

double Differential_pressure_sensor_model::diff_press_from_true_airspeed(const double true_airspeed,
        const double static_pressure, const double temperature_celsius) {
    double corrected_airspeed = corrected_airspeed_from_true_airspeed(
                                    true_airspeed,
                                    static_pressure,
                                    temperature_celsius);
...
double Differential_pressure_sensor_model::get_air_density(const double static_pressure,
