...
namespace avionics_sim
{
    ...
    Bilinear_interp::Bilinear_interp(const std::vector<std::vector<double>>& xv, const std::vector<double>& yv, const std::vector<std::vector<double>>& zv)
    {
        xVals = xv; 
        yVals = yv; 
        zVals = zv; 
    ...
    Bilinear_interp::InterpResult Bilinear_interp::interpolate(const std::vector<double>& xv, const std::vector<double>& yv, double x, double* const y)
    {
        ...
        if(x >= xv.front())
        {
            if(x <= xv.back())
            {
                ...
                std::vector<double>::const_iterator lowBound = lower_bound(xv.begin(), xv.end(), x);
    ...
    Bilinear_interp::InterpResult Bilinear_interp::interpolate2D(const std::vector<std::vector<double>>& xv, const std::vector<double>& yv, const std::vector<std::vector<double>>& zv, double x, double y, double* const z)
    {
        double y_l, y_u;
        double z_l = 0.0f; 
        double z_u = 0.0f;
        ...
        std::vector<double>::const_iterator lowBound = lower_bound(yv.begin(), yv.end(), y_clamped);
    ...
    Bilinear_interp::InterpResult Bilinear_interp::interpolate2D(double x, double y, double* const z)
    {
    ...
    bool Bilinear_interp::getX(std::vector<std::vector<double>>* const xVect)
    {
    ...
    bool Bilinear_interp::getY(std::vector<double>* const yVect)
    {
        // Return false if values havent been set. 
        if(!haveYVals)
            return false; 
    ...
    bool Bilinear_interp::getZ(std::vector<std::vector<double>>* const zVect)
    {
        // Return false if values havent been set. 
        if(!haveZVals)
            return false; 
    ...
    bool Bilinear_interp::get1DLUTelementsFromString(const std::string& inputVals, std::vector<double>* const outputVect)
    {
        ...
        while((pos = inputString.find(delimiter)) != std::string::npos)
        {
            ...
            if(sscanf(token.c_str(), "%lf", &tmpDouble) != 1)
        ...
        // Push in last value; 
        if(sscanf(inputString.c_str(), "%lf", &tmpDouble) != 1)
            return false; // Return fault if sscanf fails
    ...
    bool Bilinear_interp::get2DLUTelementsFromString(const std::string& inputVals, std::vector<std::vector<double>>* const outputVect)
    {
        // Return failure if null string. 
        if(inputVals.empty())
            return false; 
        ...
        std::vector<std::vector<double>> vect_vals; 
        std::vector<double> row; 
        ...
        while(std::getline(ss, to, ';')){
            ...
            // Push in last value; 
            if(sscanf(to.c_str(), "%lf", &tmpDouble) != 1)
                return false; // Return fault if sscanf fails
