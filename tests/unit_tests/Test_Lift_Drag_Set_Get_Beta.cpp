...
TEST_P(LiftDragSetBetaParameterized, LiftDragUnitTestSetGetBeta_UnitTest) {
	avionics_sim::Lift_drag_model ldm;
	double valueToTestAgainst;
	LiftDragSetBetaParams param=GetParam();
	...
	try
	{
		ldm.setBeta(inputAngle,param.isRadians);
		ASSERT_FLOAT_EQ(ldm.getBeta(), valueToTestAgainst);
	}
	catch(const Lift_drag_model_exception& e)
	{
		std::cerr<<"Exception successfully caught for setBeta."<<std::endl;
