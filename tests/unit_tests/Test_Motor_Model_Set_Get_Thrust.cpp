...
TEST_P(MotorModelSetThrustParameterized, MotorModelSetGetThrust_UnitTest) {
	//double valueToTestAgainst;
	MotorModelSetThrustParams param=GetParam();
	double inputThrust=param.thrust;
	double minThrust=param.minThrust;
	double maxThrust=param.maxThrust;
	...
	try
	{
		if (isnan(inputThrust))
        {
            std::string errMsg="Motor_model::setThrust: input thrust is NaN";
        ...
        else
        {
            ...
            if ( mu.rough_lt(inputThrust, minThrust, tolerance) || mu.rough_gt(inputThrust, maxThrust, tolerance) )
            {
                std::string errMsg="Thrust is not within bounds of ["+std::to_string(minThrust)+", "+std::to_string(maxThrust)+"]. rho= "+std::to_string(inputThrust);
				Motor_model_exception e(errMsg);
				throw e;
            }
        } 
	...
	{
		std::cerr<<"Exception successfully caught for setThrust."<<std::endl;
