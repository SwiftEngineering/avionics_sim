...
TEST(Differential_pressure_UnitTest, Test_indicated_from_corrected)
{
	...
	std::array<std::array<double, 6>,9> expected = {{{-2.0, 8.0, 18.0, 28.0, 38.0, 48.0}, {-1.5, 8.5, 18.5, 28.5, 38.5, 48.5},
	...
	for(size_t i = 0; i < correction.size(); i++)
	{
		for(size_t j = 0; j < corrected.size(); j++)
		{
			...
			calculated_indicated = dp.indicated_airspeed_from_corrected_airspeed(corrected[j]);
...
TEST(Differential_pressure_UnitTest, Test_corrected_airspeed_from_true_airspeed)
{
	avionics_sim::Differential_pressure_sensor_model dp(0.0);
...
;
	...
	for(size_t i = 0; i < true_airspeed.size(); i++)
	{
		for(size_t j = 0; j < static_pressure.size(); j++)
		{
			...
			{
		 		calculated_true = dp.corrected_airspeed_from_true_airspeed(true_airspeed[i], static_pressure[j], temperature[k]);
...
TEST(Differential_pressure_UnitTest, Test_compressable_vs_incompressable)
{
	...
	{
		comp_flow = dp.diff_press_from_indicated_airspeed_compressable_flow(indicated_airspeed[i]);
...
TEST(Differential_pressure_UnitTest, Test_diff_press_from_true_airspeed)
{
	...
	std::array<double,2> static_pressure = {80000, 105000};
	std::array<double,2> temperature = {-10.0, 40.0}; 
	std::array<std::array<std::array<double, 3>, 2>, 2> expected = 	{{{{{0.0,  330.905085633934448, 1323.620342535737791}, 
	...
	for(size_t i = 0; i < true_airspeed.size(); i++)
	{
		for(size_t j = 0; j < static_pressure.size(); j++)
		{
			for(size_t k = 0; k < temperature.size(); k++)
			{
				calculated_diff_press = dp.diff_press_from_true_airspeed(true_airspeed[i], static_pressure[j], temperature[k]);
